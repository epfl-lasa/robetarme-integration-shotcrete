 ...
 * as perform actions and guard conditions associated with each state
 * transition.
 */
...
private:
  ...
  public:
    /**
     * @brief Enumeration of share modes.
     */
    enum ShareMode : uint8_t { NONE = 0, FOLLOWING, AUTOMATIC };

    /**
     * @brief Get the current share mode.
     ...
     * @param share The share mode to set.
     */
  ...
  private:
    ShareMode shareMode_ = ShareMode::NONE; /**< The current share mode. */
  };
...
public:
  ...
  public:
     ...
     *
     * This operator is called when the goWorkingPosition event is triggered in
     * the FSM. It checks if the current task can go to the working position and
     * updates the FSM accordingly. If the task is successful, it sets the FSM
     * to ready state. Otherwise, it sets an error message and triggers the
     * ErrorTrigger event in the FSM.
     *
  ...
  /**
   * @brief Guard condition functor class used to determine if a task is in
   * sharing mode.
   */
  class isSharing {
  public:
    /**
     * @brief Operator() to determine if a task is in sharing mode.
     *
     * @param evt The event.
     * @param fsm The finite state machine.
     * @param src The source state.
     * @param tgt The target state.
     * @return true if the task is in sharing mode, false otherwise.
     */
    template <class EVT, class FSM, class SourceState, class TargetState>
  ...
  /**
   * @brief Guard condition functor used to check if the FSM is ready to
  ...
  public:
     ...
     *
     * This operator is called when the functor is invoked. It takes the current
     * event, FSM object, source state, and target state as arguments and
     * returns a boolean value indicating if the FSM is ready to transition.
     *
  ...
  /**
   * @brief Guard condition functor class to check if the FSM is in the "homed"
   * state.
   */
  class isHomed {
  public:
     ...
     *
     * This operator is called when evaluating the guard condition in a
     * Boost.Statechart transition. It takes the event, FSM, source state, and
     * target state as arguments and returns a boolean value indicating whether
     * the FSM is in the "homed" state or not.
     *
     * @param evt The event object.
     * @param fsm The FSM object.
     * @param src The source state object.
  ...
  public:
     ...
     *
     * This function is called by the FSM during state transitions to determine
     * if the FSM should exit.
     *
     * @param evt The event object.
     * @param fsm The FSM object.
     * @param src The source state object.
...
public:
   ...
   *
   * This state is responsible for initializing the current task. It calls the
   * `initialize()` method of the current task and processes the appropriate
   * events based on the feedback received from the task initialization. If the
   * task initialization is successful, it transitions to the Initialized state.
   * Otherwise, it sets an error message and transitions to the ErrorTrigger
   * state.
   *
   * @tparam Event The type of the event.
...
public:
   ...
   * If the path computation is successful, it transitions to the PathComputed
   * state. If the path computation fails, it sets an error message and
   * transitions to the ErrorTrigger state.
   *
   * @tparam Event The type of the event.
   * @tparam FSM The type of the state machine.
   * @param event The event that triggered the state transition.
  ...
  void on_entry(Event const &event, FSM &fsm) {
    ...
    if (feedback) {
      ...
      std::cout << "Error: Task homing failed" << std::endl;
      fsm.process_event(ErrorTrigger());
    }
  }
};
...
/**
 * @brief The HumanSharing state of the TaskFSM class.
 */
class TaskFSM::HumanSharing : public msmf::state<> {
public:
  /**
   * @brief Called when entering the HumanSharing state.
   *
  ...
  /**
   * @brief Called when exiting the HumanSharing state.
   *
   * @tparam Event The type of the event triggering the transition.
   * @tparam FSM The type of the finite state machine.
   * @param event The event triggering the transition.
...
public:
   ...
   * When entering this state, the `on_entry` function is called, which executes
   * the current task. If the task execution is successful, the state machine
   * transitions to the `Finished` state. If the task execution fails, an error
   * message is set and the state machine transitions to the `ErrorTrigger`
   * state.
   *
   * @tparam Event The type of the event.
   * @tparam FSM The type of the state machine.
  ...
  void on_entry(Event const &event, FSM &fsm) {
    ...
    if (feedback) {
      ...
      std::cout << "Error: Task execute failed" << std::endl;
...
public:
   ...
   *
   * This function executes the `goHomingPosition()` function of the current
   * task and checks the feedback. If the feedback indicates a failure, it sets
   * an error message and triggers the `ErrorTrigger` event to transition to the
   * error state.
   *
   * @tparam Event The type of the event.
   * @tparam FSM The type of the state machine.
   ...
   *
   * This function is called when transitioning out of the "Homing" state. It
   * simply prints a message indicating that the state is being left.
   *
   * @tparam Event The type of the event.
   * @tparam FSM The type of the state machine.
   * @param event The event that triggered the state transition.
